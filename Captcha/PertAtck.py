...
def attack(img, label, model, perturbation, target=None,
    ...
    bounds = [(-perturbation, +perturbation)] * img.shape[0] * img.shape[1]
...
if __name__ == "__main__":
    ...
    attack_image = attack(trainX[0], trainY[0], model, perturbation=70)

    plt.imshow(trainX[0])
    plt.show()

    for line in trainX[0]:
        print(line)

    plt.imshow(attack_image[0])
    plt.show()

    for line in attack_image[0]:
        print(line)
